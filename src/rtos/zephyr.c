 ...
 *   - Read *switch_handle to get bsa_addr
 ...
 *   - High registers (A4-A15) are saved above BSA (not accessible without window registers)
 */
static int zephyr_get_xtensa_state(struct rtos *rtos, target_addr_t *addr,
			 ...
			 struct rtos_reg **reg_list, int *num_regs)
{
	uint32_t switch_handle, bsa_addr;
	...
	retval = target_read_u32(rtos->target, switch_handle, &bsa_addr);
	if (retval != ERROR_OK)
		return retval;

	LOG_DEBUG("Zephyr Xtensa: switch_handle = 0x%08" PRIx32, switch_handle);
	LOG_DEBUG("Zephyr Xtensa: bsa_addr = 0x%08" PRIx32, bsa_addr);
	...
	/* Read PS register */
	uint32_t ps = target_buffer_get_u32(rtos->target, &bsa_data[ps_offset]);
	uint32_t pc = target_buffer_get_u32(rtos->target, &bsa_data[ps_offset + 4]);

	LOG_DEBUG("Zephyr Xtensa: PS = 0x%08" PRIx32, ps);
	LOG_DEBUG("Zephyr Xtensa: PC = 0x%08" PRIx32, pc);

	uint32_t caller_a0 = target_buffer_get_u32(rtos->target, &bsa_data[bsa_size - 16]);
	uint32_t caller_a1 = target_buffer_get_u32(rtos->target, &bsa_data[bsa_size - 12]);
	uint32_t caller_a2 = target_buffer_get_u32(rtos->target, &bsa_data[bsa_size - 8]);
	uint32_t caller_a3 = target_buffer_get_u32(rtos->target, &bsa_data[bsa_size - 4]);

	LOG_DEBUG("Zephyr Xtensa: caller_a0 = 0x%08" PRIx32, caller_a0);
	LOG_DEBUG("Zephyr Xtensa: caller_a1 = 0x%08" PRIx32, caller_a1);
	LOG_DEBUG("Zephyr Xtensa: caller_a2 = 0x%08" PRIx32, caller_a2);
	LOG_DEBUG("Zephyr Xtensa: caller_a3 = 0x%08" PRIx32, caller_a3);
	...
	*reg_list = calloc(*num_regs, sizeof(struct rtos_reg));
	...
	struct reg *reg_a1 = register_get_by_name(rtos->target->reg_cache, "ar1", 1);
	struct reg *reg_a2 = register_get_by_name(rtos->target->reg_cache, "ar2", 1);
	struct reg *reg_a3 = register_get_by_name(rtos->target->reg_cache, "ar3", 1);

	/* Update with the actual values from BSA */
	for (int i = 0; i < *num_regs; i++) {
		if (reg_pc && (*reg_list)[i].number == reg_pc->number)
			target_buffer_set_u32(rtos->target, (*reg_list)[i].value, pc);
		else if (reg_ps && (*reg_list)[i].number == reg_ps->number)
			target_buffer_set_u32(rtos->target, (*reg_list)[i].value, ps);
		else if (reg_a0 && (*reg_list)[i].number == reg_a0->number)
			target_buffer_set_u32(rtos->target, (*reg_list)[i].value, caller_a0);
		else if (reg_a1 && (*reg_list)[i].number == reg_a1->number)
			target_buffer_set_u32(rtos->target, (*reg_list)[i].value, caller_a1);
		else if (reg_a2 && (*reg_list)[i].number == reg_a2->number)
			target_buffer_set_u32(rtos->target, (*reg_list)[i].value, caller_a2);
		else if (reg_a3 && (*reg_list)[i].number == reg_a3->number)
			target_buffer_set_u32(rtos->target, (*reg_list)[i].value, caller_a3);
	}
